...
| Tool calls hang or return errors                      | Editor may be busy compiling, loading a level, or in PIE. Wait and retry. For eligible implementation-only changes in existing `.cpp` files, prefer `LiveCodingToolset.CompileLiveCoding`. Object Reinstancing can patch broader changes, but structural changes and `data type changes` warnings require a full build and editor restart before final validation. |
