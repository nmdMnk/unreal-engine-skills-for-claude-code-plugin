...
- **Choose the right C++ build.** Follow the project's build policy when present. Live Coding can technically compile headers, new classes, and structural or reflected changes through Object Reinstancing, but that is not equivalent to reliable final validation. To rebuild C++ from the running editor, drive `LiveCodingToolset.CompileLiveCoding` and wait on its result instead of asking the user to switch to the IDE. Use `LiveCodingToolset.CompileLiveCoding` as the reliable fast path for implementation-only changes in existing `.cpp` files from already loaded modules, including edits to existing functions and non-exported file-local helpers, provided they do not introduce static/global initialization or alter declarations, ABI/layout, reflection, the C++ file set, or the module graph. Wait for its result and fix compiler errors. Header or reflected-declaration changes (`UCLASS`, `USTRUCT`, `UENUM`, `UFUNCTION`, `UPROPERTY`, `UDELEGATE`, or generated-body layout), constructor or class-default changes, module dependencies, and added/removed/renamed C++ files require the project's normal full build and editor restart before final validation. Treat a Live Coding `Success` that warns about `data type changes` as provisional, not final validation. That tool blocks until the compile actually finishes and surfaces MSVC diagnostics.
